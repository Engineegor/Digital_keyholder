...
static key_state_t min_state_of_mask(key_bitmask_t keys, bool peek = false) {
	...
	for(key_id_t i = (key_id_t)0; i < KEY_MAX_INVALID; i = (key_id_t) (i + 1)) {
		...
		if(maxTimeStamp < keypress_started_at[i]) {maxTimeStamp = keypress_started_at[i];}
	}
	...
	switch(preliminary_state) {
		...
		case KEYSTATE_PRESSED:
			if(pressed_keycombos.count(keys) == 0) {
				if(!peek) {pressed_keycombos.insert(keys);}
				return KEYSTATE_HIT;
			} else {return KEYSTATE_PRESSED;}
...
void hid_set_key_state(key_id_t key, bool state) {
    if(state && (active_keys & key) == 0) {
        keypress_started_at[key] = xTaskGetTickCount();
        keypress_repeated_at[key] = xTaskGetTickCount();
        active_keys |= key;
    } else if(!state && (active_keys & key) != 0) {
        active_keys &= ~key;
    }
}

key_state_t hid_check_key_state(key_id_t key) {return hid_check_key_all(key);}
...
key_state_t hid_check_key_any(key_bitmask_t keys) {
...
key_state_t hid_check_key_state_repetition(key_id_t key) {
    switch(hid_check_key_state(key)) {
        case KEYSTATE_RELEASED:
        case KEYSTATE_PRESSED: 
            return KEYSTATE_RELEASED;
        case KEYSTATE_HIT:
